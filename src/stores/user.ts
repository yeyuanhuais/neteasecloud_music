...
export const useUserStore = defineStore("user", () => {
  ...
  const checkLogin = async () => {
    const { data } = await _axios.get<{ data: { profile: UserProfile; account: UserAccount; code: number } }, any>("/login/status").catch(err => {});
    if (data.code === 200) {
  ...
  const onLoginPhone = async (params: any) => {
    const { code, msg } = await _axios.get<{ code: number; cookie: string; message: string }, any>("/login/cellphone", { params }).catch(err => {});
    if (code === 200) {
      checkLogin();
    } else {
      ElMessage({
  ...
  const onLoginEmail = async (params: any) => {
    const { code, msg } = await _axios.get<{ code: number; cookie: string; message: string }, any>("/login", { params }).catch(err => {});
    if (code === 200) {
      checkLogin();
    } else {
      ElMessage({
