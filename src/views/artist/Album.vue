...
const { getArtistAlbum } = useArtistStore();
...
const getData = async () => {
  try {
    ...
    const { hotAlbums } = await getArtistAlbum(props.id, pageData.limit, offset.value);
    if (pageData.page === 1) {
...
<template>
  <div class="mt-5">
    <div class="grid grid-flow-row grid-cols-4 lg:grid-cols-6 gap-5 2xl:grid-cols-8">
      <div v-for="(item, index) in list" :key="index" :class="{ 'item-1': index === 0 }" @click="router.push({ name: 'album', query: { id: item.id } })" class="hover-text">
