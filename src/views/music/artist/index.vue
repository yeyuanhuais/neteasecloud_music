...
const { getArtistList } = useArtistStore();

const getData = async () => {
  pageData.loading = true;
  try {
    const data = await getArtistList(pageData);
    if (pageData.page === 1) {
      ...
      <div class="ml-3">
        <el-space wrap :size="10" :spacer="spacer">
          <div
            ...
            :class="{
              active: (item.key === pageData.type && option.key === 'type') || (item.key === pageData.area && option.key === 'area') || (item.key === pageData.initial && option.key === 'initial'),
            }"
